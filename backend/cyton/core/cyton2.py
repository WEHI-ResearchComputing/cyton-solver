...
class Cyton2Model:
    ...
    def compute_pdf(self, times: ExtrapolationTimes, mu: float, sig: float):
        if self.logn:
            return lognorm.pdf(times, sig, scale=mu)
        else:
            return norm.pdf(times, mu, sig)

    def compute_cdf(self, times: ExtrapolationTimes, mu: float, sig: float):
        if self.logn:
            return lognorm.cdf(times, sig, scale=mu)
        else:
            return norm.cdf(times, mu, sig)

    def compute_sf(self, times: ExtrapolationTimes, mu: float, sig: float):
        if self.logn:
    ...
    def evaluate(
    ...
    ):  # subsequent division time, activation probability
        times = self.times
        ...
        nUNS = self.n0 * (1.0 - params["p"]) * sfUns
        nDIV[0, :] = self.n0 * params["p"] * sfDie * sfDiv * sfDD
        nDES[0, :] = (
            self.n0
            * params["p"]
            * sfDie
            * np.cumsum([x * y for x, y in zip(pdfDD, sfDiv)])
            * self.dt
        )
        ...
        for igen in range(1, self.max_div + 1):
            core = 2.0**igen * self.n0 * params["p"]
            upp_cdfDiv = self.compute_cdf(
                times - ((igen - 1.0) * params["b"]), params["mDiv0"], params["sDiv0"]
            )
            low_cdfDiv = self.compute_cdf(
                times - (igen * params["b"]), params["mDiv0"], params["sDiv0"]
            )
            ...
            nDES[igen, :] = (
                core
                * sfDie
                * np.cumsum([x * y for x, y in zip(pdfDD, difference)])
                * self.dt
            )
    ...
    def extrapolate(
        ...
        # Calculate gen > 0 cells
        for igen in range(1, self.max_div + 1):
            core = 2.0**igen * self.n0 * p

            upp_cdfDiv = self.compute_cdf(
                model_times - ((igen - 1.0) * b), mDiv0, sDiv0
            )
            ...
            if igen < self.exp_max_div:
                ...
                cells_gen[self.exp_max_div, :] += nDIV[igen, :] + nDES[igen, :]
        total_live_cells = np.sum(
            cells_gen, axis=0
        )  # sum over all generations per time point
